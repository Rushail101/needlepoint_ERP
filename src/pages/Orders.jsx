...
export default function Orders() {
  ...
  // 1. Group items sharing the same po_number into batch objects
  const groupedOrders = (() => {
    const groups = []
    const poMap = new Map()
    ...
    return groups.map(g => {
      // If a batch only has 1 item, flatten it to a single item card
  ...
  // 2. Filter & Sort (Pushes completed/packed batches and orders to bottom)
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {filtered.map((item) => {
            // RENDER MULTI-PRODUCT BATCH CARD
            if (item.isBatch) {
        ...
        <Modal onClose={() => setSelectedBatch(null)}>
          <div className="mb-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-100">Order Batch: {selectedBatch.po_number}</h3>
              <span className="text-xs bg-brand-950 text-brand-300 border border-brand-800 px-2 py-0.5 rounded-full font-semibold">
                {selectedBatch.items.length} Styles
              </span>
            </div>
            <p className="text-xs text-gray-400 mt-0.5">{selectedBatch.brands?.name || 'Independent'}</p>
          </div>

          <p className="text-xs text-gray-500 mb-2 font-medium">Select a product to view lot status or update details:</p>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">{prod.style_code ? `Style: ${prod.style_code} · ` : ''}{qty} pcs</p>
